...
export const AdminDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'events' | 'announcements' | 'gallery' | 'comments' | 'applications'>('dashboard');
  const [mobileSidebarOpen, setMobileSidebarOpen] = useState(false);

  const handleAction = (actionName: string) => {
    showToast(`"${actionName}" triggered. Backend functionality will be connected later.`, 'info');
  };

  const handleLogout = () => {
    showToast('Logged out of demo admin session.', 'info');
  ...
  const sidebarLinks = [
    ...
    { id: 'gallery', label: 'Gallery', icon: <ImageIcon className="w-4 h-4" />, count: mockAdminStats.galleryImages },
        ...
        <header className="bg-white border-b border-slate-200 px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500">
              CSI CMRIT Student Chapter Administration Console (UI Prototype)
            </p>
          </div>
        ...
        <div className="p-6 space-y-8 flex-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-subtle flex flex-col justify-between">
              ...
              <div className="text-[11px] text-emerald-600 font-medium mt-1">Active schedule</div>
            </div>
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-subtle flex flex-col justify-between">
              ...
              <div className="text-[11px] text-slate-500 font-medium mt-1">Official circulars</div>
            </div>

            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-subtle flex flex-col justify-between">
              <div className="flex items-center justify-between text-slate-500 mb-2">
                <span className="text-xs font-semibold uppercase tracking-wider">Gallery Media</span>
                <ImageIcon className="w-4 h-4 text-emerald-600" />
              </div>
              <div className="text-2xl font-extrabold text-slate-900">{mockAdminStats.galleryImages}</div>
              <div className="text-[11px] text-slate-500 font-medium mt-1">Photos & assets</div>
            </div>

            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-subtle flex flex-col justify-between">
              <div className="flex items-center justify-between text-slate-500 mb-2">
                <span className="text-xs font-semibold uppercase tracking-wider">Pending Comments</span>
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-subtle flex flex-col justify-between">
              ...
              <div className="text-[11px] text-blue-600 font-medium mt-1">Membership drive</div>
            </div>
          </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-subtle overflow-hidden">
              <div className="p-5 border-b border-slate-100 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-500">Scheduled workshops, hackathons, and symposiums</p>
                </div>
                <Button
                  variant="accent"
                  size="sm"
                    ...
                    {mockEvents.slice(0, 4).map((evt) => (
                      <tr key={evt.id} className="hover:bg-slate-50/60 transition-colors">
                        ...
                        <td className="px-5 py-3">
                          <Badge variant="blue">{evt.category}</Badge>
                        </td>
                        <td className="px-5 py-3 text-slate-600">{evt.date}</td>
                        <td className="px-5 py-3">
                          <span
                        ...
                        <td className="px-5 py-3 text-right space-x-2">
                          <button
                            onClick={() => handleAction(`Edit ${evt.title}`)}
                          ...
                          <button
                            onClick={() => handleAction(`Delete ${evt.title}`)}
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-subtle overflow-hidden">
              <div className="p-5 border-b border-slate-100 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-500">Official circulars, recruitment notices, and alerts</p>
                </div>
                <Button
                  variant="accent"
                  size="sm"
                  leftIcon={<Plus className="w-3.5 h-3.5" />}
                  onClick={() => handleAction('Add Announcement')}
                >
                  New Announcement
                </Button>
              </div>
                  ...
                  <tbody className="divide-y divide-slate-100 text-slate-700">
                    {mockAnnouncements.slice(0, 3).map((ann) => (
                      <tr key={ann.id} className="hover:bg-slate-50/60 transition-colors">
                        <td className="px-5 py-3 font-semibold text-slate-900 max-w-sm truncate">
                          {ann.title}
                        </td>
                        <td className="px-5 py-3">
                          <Badge variant="slate">{ann.category}</Badge>
                        </td>
                        <td className="px-5 py-3 text-slate-600">{ann.date}</td>
                        <td className="px-5 py-3 text-slate-600">{ann.author}</td>
                        <td className="px-5 py-3 text-right space-x-2">
                          <button
                            onClick={() => handleAction(`Edit ${ann.title}`)}
                            className="p-1 text-slate-500 hover:text-blue-600 hover:bg-slate-100 rounded"
                            title="Edit"
                          >
                            <Pencil className="w-3.5 h-3.5" />
                          ...
                          <button
                            onClick={() => handleAction(`Delete ${ann.title}`)}
                            className="p-1 text-slate-500 hover:text-rose-600 hover:bg-rose-50 rounded"
                            title="Delete"
                          >
                            <Trash2 className="w-3.5 h-3.5" />
          ...
          {/* Table 3: Pending Comments & Moderation */}
          {(activeTab === 'dashboard' || activeTab === 'comments') && (
            <div className="bg-white rounded-xl border border-slate-200 shadow-subtle overflow-hidden">
              <div className="p-5 border-b border-slate-100 flex items-center justify-between">
                <div>
                  <h3 className="text-base font-bold text-slate-900">Pending Inquiries & Comments</h3>
                  <p className="text-xs text-slate-500">Student queries awaiting moderation</p>
                </div>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs">
                  <thead className="bg-slate-50 text-slate-500 font-semibold uppercase border-b border-slate-200/80">
                    <tr>
                      <th className="px-5 py-3">Author</th>
                      <th className="px-5 py-3">Related Event/Post</th>
          ...
          {/* Table 4: Recent Join Applications */}
          {(activeTab === 'dashboard' || activeTab === 'applications') && (
            <div className="bg-white rounded-xl border border-slate-200 shadow-subtle overflow-hidden">
              <div className="p-5 border-b border-slate-100 flex items-center justify-between">
                <div>
                  <h3 className="text-base font-bold text-slate-900">Recent Join Applications</h3>
                  <p className="text-xs text-slate-500">Student submissions from the 2026 membership drive</p>
                </div>
                <Button
                  variant="outline"
                  size="sm"
                  ...
                  <tbody className="divide-y divide-slate-100 text-slate-700">
                    {mockRecentApplications.map((app) => (
                      <tr key={app.id} className="hover:bg-slate-50/60 transition-colors">
                        ...
                        <td className="px-5 py-3 text-right space-x-2 whitespace-nowrap">
                          <button
                            onClick={() => handleAction(`Approve application: ${app.fullName}`)}
                          ...
                          <button
                            onClick={() => handleAction(`Reject application: ${app.fullName}`)}
